    ...
    <div className="glass rounded-xl p-4">
      <div className="flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={onSave}
          className="flex items-center gap-1 rounded-md border border-white/15 px-3 py-1.5 text-[13px] font-semibold text-neutral-300 hover:border-white/35 hover:text-neutral-100"
        >
          ...
          {saved.map((item) => (
            <li key={item.id} className="flex items-center justify-between py-2">
              <button
                type="button"
                onClick={() => onLoad(item)}
                className="truncate text-left text-[15px] text-neutral-300 hover:text-neutral-100 hover:underline"
                title={item.name}
              >
                {item.name}
              </button>
